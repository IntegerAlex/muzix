...
import { BG, TEXT_PRIMARY, TEXT_SECONDARY, TEXT_MUTED, ACCENT, SURFACE_ELEVATED, SURFACE_ICON, BORDER, SURFACE } from '@/lib/colors';
...
export default function LibraryScreen() {
  ...
  const [activeCategory, setActiveCategory] = useState<'all' | 'playlists' | 'artists' | 'albums'>('all');

  const sidebarWidth = isDesktop ? 200 : 72;

  const categories = [
    { key: 'all' as const, label: 'All', Icon: Music },
    { key: 'playlists' as const, label: 'Playlists', Icon: ListMusic },
    { key: 'artists' as const, label: 'Artists', Icon: User },
    { key: 'albums' as const, label: 'Albums', Icon: Music },
  ];
  ...
  const contentArea = (filtered: boolean) => (
    <>
      ...
      {error ? (
        ...
        <>
          {(!filtered || activeCategory === 'all' || activeCategory === 'playlists') && renderPlaylistsSection()}
          {(!filtered || activeCategory === 'all' || activeCategory === 'artists') && renderArtistsSection()}
          {(!filtered || activeCategory === 'all' || activeCategory === 'albums') && renderAlbumsSection()}
        </>
      )}
    </>
  );
  ...
  if (isWide) {
    return (
      <View style={{ flex: 1, flexDirection: 'row', backgroundColor: BG }}>
        <View style={{ width: sidebarWidth, backgroundColor: SURFACE, borderRightWidth: 1, borderRightColor: BORDER, paddingTop: 64 }}>
          {categories.map((cat) => (
            <Pressable
              key={cat.key}
            ...
            >
              <cat.Icon size={22} color={activeCategory === cat.key ? ACCENT : TEXT_SECONDARY} />
              {isDesktop && (
                <Text style={{ fontSize: 14, fontWeight: '600', color: activeCategory === cat.key ? ACCENT : TEXT_SECONDARY }}>
                  {cat.label}
                </Text>
              )}
            </Pressable>
